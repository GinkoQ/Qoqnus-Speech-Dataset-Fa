"""
Standalone builder for seyedali dataset.
Run from v2/: python builders/seyedali.py

Source: raw/seyedali/data/*.parquet
  columns : ['audio_id', 'audio', 'speaker_id', 'gender', 'emotion', 'transcript', 'ipa']
  audio   : AudioDecoder (_hf_encoded bytes, wav)
  text    : transcript
  speaker : speaker_id col (dataset-provided) -> tagged as "ds:<speaker_id>"
  splits  : test, train (no validation)

Smoke test: python builders/seyedali.py --smoke 5
"""
import os
...
SOURCE      = "seyedali"
RAW_DIR     = Path("raw/seyedali")
OUT_DIR     = Path("processed/seyedali")
SHARDS_DIR  = OUT_DIR / "_shards"
LOG_DIR     = Path("logs")
TEXT_COL    = "transcript"
SPEAKER_COL = "speaker_id"   # dataset-provided
TARGET_SR   = 16000
MIN_DUR     = 0.3
MAX_DUR     = 40.0
CLIP_AMP    = 0.999
...
def process_row(args):
    ...
    # speaker_id from dataset, tagged with "ds:" prefix
    spk_raw = str(row.get(SPEAKER_COL) or "").strip()
    speaker_id = f"ds:{spk_raw}" if spk_raw else f"ds:{SOURCE}_unknown"
    ns = len(arr); dur = ns / TARGET_SR; nw = len(text.split())
    return {
        "utt_id": utt_id, "audio_flat": arr, "sampling_rate": TARGET_SR,
        "text": text, "duration": float(round(dur,4)), "n_samples": ns,
        "speaker_id": speaker_id, "snr_db": float(snr_db(arr)),
...
def process_split(split, files, all_drops, smoke=0):
    ...
    rows = [dict(r) for r in ds]; del ds
    tasks = [(i, row, split) for i, row in enumerate(rows)]; del rows
...
def assemble_split(split):
    ...
    ds = load_dataset("parquet", data_files=[str(p) for p in shards], split="train", num_proc=min(8,os.cpu_count() or 1))
    ds = ds.map(lambda b: {"audio": [{"array": a, "sampling_rate": s} for a,s in zip(b["audio_flat"],b["sampling_rate"])]},
                batched=True, batch_size=1024, num_proc=min(8,os.cpu_count() or 1), desc=f"  rebuilding audio [{split}]")
    return ds.remove_columns(["audio_flat"])
...
def build(smoke=0):
    ...
    if all_drops:
        ...
        print(f"  drop log: {LOG_DIR}/{SOURCE}_dropped.jsonl ({len(all_drops)})")
    ...
    for split, ds in dd_v.items():
        assert all(v==TARGET_SR for v in ds["sampling_rate"])
...
if __name__ == "__main__":
    ...
    ap.add_argument("--smoke", type=int, default=0, help="rows per split for smoke test")
    args = ap.parse_args()
