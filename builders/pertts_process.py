"""
Process pertts dataset from built parquet into unified format.
Run from v2/: python builders/pertts_process.py

Prerequisite: python builders/pertts.py  (builds raw/pertts/train.parquet)

Source: raw/pertts/train.parquet
  columns : ['audio', 'sentence']
  audio   : dict {'bytes': <wav bytes>, 'path': None}  (self-contained)
  text    : sentence
  speaker : no speaker col -> gen:pertts_spk0000
  splits  : train only

Smoke test: python builders/pertts_process.py --smoke 5
"""
import os
...
SOURCE      = "pertts"
RAW_FILE    = Path("raw/pertts/train.parquet")
OUT_DIR     = Path("processed/pertts")
SHARDS_DIR  = OUT_DIR / "_shards"
LOG_DIR     = Path("logs")
TEXT_COL    = "sentence"
SPEAKER     = "gen:pertts_spk0000"
TARGET_SR   = 16000
MIN_DUR     = 0.3
MAX_DUR     = 40.0
CLIP_AMP    = 0.999
...
def decode_audio(row):
    # audio is dict {'bytes': <bytes>, 'path': None}
    audio = row["audio"]
    if isinstance(audio, dict):
        ...
        if p and Path(p).exists():
            ...
            return np.asarray(arr, dtype=np.float32), int(sr)
    raise ValueError(f"no usable audio in dict: keys={list(audio.keys()) if isinstance(audio,dict) else type(audio)}")
...
def build(smoke=0):
    t0 = time.time()
    print(f"\n{'='*60}\n[{SOURCE}] process builder{'  [SMOKE]' if smoke else ''}\n{'='*60}")
    if not RAW_FILE.exists():
        raise FileNotFoundError(f"not found: {RAW_FILE}\nRun: python builders/pertts.py first")
    ...
    ds = load_dataset("parquet", data_files=str(RAW_FILE), split="train")
    ...
    ds_out = ds_out.map(lambda b: {"audio": [{"array": a, "sampling_rate": s} for a,s in zip(b["audio_flat"],b["sampling_rate"])]},
                        batched=True, batch_size=1024, num_proc=min(8,os.cpu_count() or 1), desc="  rebuilding audio")
    ds_out = ds_out.remove_columns(["audio_flat"])
    ...
    ds_v = dd_v["train"]
    assert all(v==TARGET_SR for v in ds_v["sampling_rate"])
