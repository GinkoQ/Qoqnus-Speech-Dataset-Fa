"""
Standalone builder for pourmand dataset.
Run from v2/: python builders/pourmand.py

Source: raw/pourmand/data/*.parquet
  columns : ['audio', 'video_id', 'segment_id', 'title', 'transcription', 'youtube_url']
  audio   : AudioDecoder (_hf_encoded bytes, mp3)
  text    : transcription
  speaker : no speaker col -> gen:pourmand_spk0000 (single pseudo-speaker)
  splits  : test, train, validation

Smoke test: python builders/pourmand.py --smoke 5
"""
import os
...
SOURCE      = "pourmand"
RAW_DIR     = Path("raw/pourmand")
OUT_DIR     = Path("processed/pourmand")
SHARDS_DIR  = OUT_DIR / "_shards"
LOG_DIR     = Path("logs")
TEXT_COL    = "transcription"
SPEAKER     = "gen:pourmand_spk0000"   # no speaker info in dataset
TARGET_SR   = 16000
MIN_DUR     = 0.3
MAX_DUR     = 40.0
CLIP_AMP    = 0.999
...
def process_row(args):
    ...
    return {
        "utt_id": utt_id,
        "audio": {"array": arr.tolist(), "sampling_rate": TARGET_SR},
...
def process_split(split, files, all_drops, smoke=0):
    ...
    with ThreadPoolExecutor(max_workers=NUM_WORKERS) as pool:
        row_idx = 0
        for chunk_batch in ds.iter(batch_size=CHUNK):
            keys = list(chunk_batch.keys())
            chunk_size = len(chunk_batch[keys[0]])
            chunk_rows = [{k: chunk_batch[k][i] for k in keys} for i in range(chunk_size)]
            tasks = [(row_idx + i, row, split) for i, row in enumerate(chunk_rows)]
            row_idx += chunk_size
            del chunk_rows
...
def assemble_split(split):
    ...
    # Audio struct is already embedded - just load directly, no map needed
    ds = load_dataset("parquet", data_files=[str(p) for p in shards], split="train",
                      num_proc=1)
    return ds
