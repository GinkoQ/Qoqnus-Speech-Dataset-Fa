"""
Standalone builder for mshojaei dataset.
Run from v2/: python builders/mshojaei.py

Source: raw/mshojaei/data/*.parquet
  columns : ['audio', 'sentence']
  audio   : AudioDecoder (_hf_encoded bytes, mp3)
  text    : sentence
  speaker : no speaker col -> gen:mshojaei_spk0000
  splits  : train only

Smoke test: python builders/mshojaei.py --smoke 5
"""
import os
...
SOURCE      = "mshojaei"
RAW_DIR     = Path("raw/mshojaei")
OUT_DIR     = Path("processed/mshojaei")
SHARDS_DIR  = OUT_DIR / "_shards"
LOG_DIR     = Path("logs")
TEXT_COL    = "sentence"
SPEAKER     = "gen:mshojaei_spk0000"
TARGET_SR   = 16000
MIN_DUR     = 0.3
MAX_DUR     = 40.0
CLIP_AMP    = 0.999
...
def process_audio(arr, sr):
    ...
    if int(clipped.sum()) >= min_s:
        ...
        if int(((cs[min_s:] - cs[:-min_s]) >= min_s).sum()) > 0: raise ValueError("clipping detected")
    return arr
...
def process_row(args):
    ...
    return {
        "utt_id": utt_id, "audio": {"array": arr, "sampling_rate": TARGET_SR},
...
def build(smoke=0):
    ...
    for split, ds in dd_v.items():
        ...
        for r in sample:
            assert r["audio"]["sampling_rate"] == TARGET_SR
            assert r["n_samples"] == len(r["audio"]["array"])
            assert not (r["audio"].get("path") and str(r["audio"]["path"]).strip())
